...
var userprops = load({}, 'userprops.js');
    ...
    var padding = (console.screen_columns - 80) / 2;
    var leftpad = lpad('',padding);
...
while (bbs.online) {
    ...
    if (defPage == '1') {
              console.clear();
              if(console.screen_rows>35)
              dittoMenu('header');
                
            dittoMenu(options.fontcode);
			console.print(format(leftpad + "%sSettings for \1h\1w%s #%d%s, Page %s1\r\n"
					,color.normal
					,user.alias
					,user.number
					,color.normal
            ...
            console.putmsg(lpad(leftpad + color.white + 'USER ACCOUNT SETTINGS\1n',64));
            console.crlf();
            console.putmsg(leftpad + bracket('A') + color.normal + ' Terminal Modes' + color.bright + '                 : \1n');
            ...
            console.crlf();
            console.putmsg(leftpad + bracket('B') + color.normal + ' External Editor' + color.bright + '                : \1n'
                    + color.bright + xtrn_area.editor[user.editor].name);
            console.crlf();
            console.putmsg(leftpad + bracket('C') + color.normal + ' Screen Length' + color.bright + '                  : \1n');
            if (user.screen_rows)
            ...
            console.putmsg(leftpad + bracket('D') + color.normal + ' Current Command Shell' + color.bright + '          : \1n');
            console.putmsg(color.bright + user.command_shell.toUpperCase());
            console.crlf();

            console.putmsg(leftpad + bracket('E') + color.normal + ' Expert Mode' + color.bright + '                    : \1n');
            console.putmsg((user.settings & USER_EXPERT) ? color.bright + 'ON\1n' : color.dark + 'OFF\1n');
            console.crlf();
            console.putmsg(leftpad + bracket('F') + color.normal + ' Screen Pause' + color.bright + '                   : \1n');
            console.putmsg((user.settings & USER_PAUSE) ? color.bright + 'ON\1n' : color.dark + 'OFF\1n');
            console.crlf();
            console.putmsg(leftpad + bracket('G') + color.normal + ' Hot Keys' + color.bright + '                       : \1n');
            console.putmsg((user.settings & USER_COLDKEYS) ? color.dark + 'OFF\1n' : color.bright + 'ON\1n');
            console.crlf();
            console.putmsg(leftpad + bracket('H') + color.normal + ' Spinning Cursor' + color.bright + '                : \1n');
            console.putmsg((user.settings & USER_SPIN) ? color.bright + 'ON\1n' : color.dark + 'OFF\1n');
            console.crlf();
            console.putmsg(leftpad + bracket('I') + color.normal + ' Default to Quiet Mode' + color.bright + '          : \1n');
            console.putmsg((user.settings & USER_QUIET) ? color.bright + 'ON\1n' : color.dark + 'OFF\1n');
            console.crlf();
            console.putmsg(leftpad + bracket('J') + color.normal + ' Fast Login Default Option' + color.bright + '      : \1n');
			...
			console.putmsg(leftpad + bracket('P') + color.normal + ' 2FA' + color.bright + '                            : ' + twofa + '\1n');
			console.crlf();
            console.putmsg(lpad(color.white + 'USER PROFILE SETTINGS\1n',64));
            console.crlf();
			// [K] Update Location/Affiliations
            console.print(format(leftpad + "%s %sUpdate Location/Affiliations%s%-3s: \1n"
					,bracket('K')
					,color.normal
					,color.bright
					,' '));
			...
			// [L] Change Password
            console.print(format(leftpad + "%s %sChange Password\1n\r\n"
					,bracket('L')
					,color.normal));

			// [M] Change Internet E-mail
            console.print(format(leftpad + "%s%s Change Internet E-mail %s%-8s: \1n"
					,bracket('M')
					,color.normal
					,color.bright
					,' '));
...
//					,' '));
 //           console.putmsg(color.dark + user.email + '\1n\r\n');
			...
			// [N] Update Signature
            console.print(format(leftpad + "%s %sUpdate Signature\1n\r\n"
					,bracket('N')
					,color.normal));
					
			// [-] Birthdate (READ ONLY)
			console.print(format(leftpad + "%s%s Birthdate %s(READ ONLY)%s%-10s: \1n"
					,bracket(color.normal + '-')
					,color.normal
					,color.dark
					,color.bright
			...
			// [O] Update Gender
			console.print(format(leftpad + "%s %sGender Identity%s%-16s: \1n"
					,bracket('O')
					,color.normal
					,color.bright
					,' '));
            ...
            // [FOOTER - PAGE ONE]
            console.gotoxy(1, 22);
			console.print(leftpad + format('%s %suit %s%s To Switch Pages.\r\n\r\n'
						,bracket('Q')
						,color.normal
						,bracket('ARROWS' + color.dark + ',' + color.bright + '#')
						,color.normal));
						
			dittoPrompt('Defaults');
            ...
            // [HEADER - PAGE TWO]
			console.print(format(leftpad + "%sSettings for \1h\1w%s #%d%s, Page %s2\r\n"
					,color.normal
					,user.alias
					,user.number
					,color.normal
            ...
            console.putmsg(lpad(leftpad + color.white + 'CHAT SETTINGS\1n',56));
            console.crlf();
			
			// [A] Allow Paging
			console.print(format(leftpad + "%s %sAllow Paging%s%-19s: \1n"
					,bracket('A')
					,color.normal
					,color.bright
					,' '));
			...
			// [B] Activity Alerts
			console.print(format(leftpad + "%s %sActivity Alerts%s%-16s: \1n"
					,bracket('B')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sPrivate Split-Screen Chat%s%-6s: \1n"
					,bracket('C')
					,color.normal
					,color.bright
					,' '));
			...
			console.crlf();
            console.putmsg(lpad(leftpad + color.white + 'MESSAGE SCAN SETTINGS\1n',64));
            console.crlf();
			
			// [D] Ask For Your Un-read Msg Scan
			console.print(format(leftpad + "%s %s'New Scan' At Login%s%-12s: \1n"
					,bracket('D')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %s'Un-read' Msg Scan At Login%s%-4s: \1n"
					,bracket('E')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sNew Scan Configuration\1n\r\n"
					,bracket('F')
					,color.normal));				
			
			// [G] Your "to-you" Message Scan Config
			console.print(format(leftpad + "%s %sYour 'to-you' Message Scan Config\1n\r\n"
					,bracket('G')
					,color.normal));				
			
			// [H] Re-init New Scan Pointers
			console.print(format(leftpad + "%s %sRe-init New Scan Pointers\1n\r\n"
					,bracket('H')
					,color.normal));			
			
			// [I] Set New Scan Pointers
			console.print(format(leftpad + "%s %sSet New Scan Pointers\1n\r\n"
					,bracket('I')
					,color.normal));
			console.crlf();
            console.putmsg(lpad(leftpad + color.white + 'MESSAGE READ SETTINGS\1n',64));
            console.crlf();
			
			// [J] Remember Current Sub-Board
			console.print(format(leftpad + "%s %sRemember Current Sub-Board%s%-5s: \1n"
					,bracket('J')
					,color.normal
					,color.bright
					,' '));
            ...
            console.print(format(leftpad + "%s %sClear Screen Between Messages%s%-2s: \1n"
					,bracket('K')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sForward Email to Netmail%s%-7s: \1n"
					,bracket('L')
					,color.normal
					,color.bright
					,' '));
            ...
            console.gotoxy(1, 22);
			console.print(format(leftpad + '%s %suit %s%s To Switch Pages.\r\n\r\n'
						,bracket('Q')
						,color.normal
						,bracket('ARROWS' + color.dark + ',' + color.bright + '#')
						,color.normal));
        ...
        if(console.screen_rows>35)
            dittoMenu('header');
            // [HEADER - PAGE THREE]
			console.print(format(leftpad + "%sSettings for \1h\1w%s #%d%s, Page %s3\r\n"
					,color.normal
					,user.alias
					,user.number
					,color.normal
            ...
            console.putmsg(lpad(leftpad + color.white + 'APPEARANCE PREFERENCES\1n',65));
			console.crlf();
			// [A] Random Case
            console.putmsg(leftpad + bracket('A') + color.normal + dittoRandomCase(' RaNDOm CAsE') + color.bright + '                    : \1n');
            console.putmsg((user.security.flags2 & UFLAG_R) ? color.bright + 'YES\1n' : color.dark + 'NO\1n');
			console.crlf();
			// [B] Leet Case
            console.putmsg(leftpad + bracket('B') + color.normal + dittoLeetCase(' lEET cASE') + color.bright + '                      : \1n');
            console.putmsg((user.security.flags2 & UFLAG_E) ? color.bright + 'YES\1n' : color.dark + 'NO\1n');
			console.crlf();
			// [C] Random Color
            console.putmsg(leftpad + bracket('C') + dittoRandomColor(' Random Color') + color.bright + '                   : \1n');
            console.putmsg((user.security.flags2 & UFLAG_B) ? color.bright + 'YES\1n' : color.dark + 'NO\1n');
			console.crlf();
			// [D] Leet Color
			console.putmsg(leftpad + bracket('D') + dittoLeetColor(' Leet Color') + color.bright + '                     : \1n');
            console.putmsg((user.security.flags2 & UFLAG_C) ? color.bright + 'YES\1n' : color.dark + 'NO\1n');
			console.crlf();
			// [E] Regular Color
            console.putmsg(leftpad + bracket('E')  + ' Reset Text' + color.bright + '                     : \1n');
            console.putmsg((user.security.flags2 & UFLAG_G) ? color.bright + 'YES\1n' : color.dark + 'NO\1n');
			console.crlf(2);
			console.putmsg(leftpad + color.normal + dittoText(" This is a piece of sample text."));
			//console.crlf(2);
			//console.putmsg(LIGHTGRAY + " Cannot reset text while Random or Leet case is enabled.")
			console.crlf(2);
            console.putmsg(lpad(leftpad + color.white + 'FILE AREA OPTIONS\1n',60));
            console.crlf();
			// [F] Default Download Protocol
			console.print(format(leftpad + "%s %sDefault Download Protocol%s%-6s: \1n"
					,bracket('F')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sAuto New File Scan At Login%s%-4s: \1n"
					,bracket('G')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sBatch Download File tagging%s%-4s: \1n"
					,bracket('H')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sSet New-Scan From-Date%s%-9s: \1n"
					,bracket('I')
					,color.normal
					,color.bright
					,' '));
			...
			console.print(format(leftpad + "%s %sToggle Extended Descriptions%s%-3s: \1n"
					,bracket('J')
					,color.normal
					,color.bright
					,' '));
